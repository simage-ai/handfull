...
export function MealDetailDialog({
  ...
  // Filter for display - only show non-zero values
        ...
        <DialogHeader>
          <DialogTitle className="space-y-1">
            <div className="flex items-center gap-2">
              {meal.mealCategory && (
                <Badge variant="secondary">{formatMealCategory(meal.mealCategory)}</Badge>
              )}
            ...
            {nonZeroMacros.length > 0 ? (
              <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
                {nonZeroMacros.map(({ key, value }) => (
                  ...
                  >
                    <div className={`text-2xl font-bold ${MACRO_CONFIG[key].text}`}>
                      {value}
                    </div>
                    <div className={`text-xs font-medium ${MACRO_CONFIG[key].text} opacity-80`}>
